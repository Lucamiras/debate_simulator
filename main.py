...
if st.button("Start debate") and user_input_debate_topic and user_input_style:
    ...
    with st.spinner("Thinking about my opening statement ..."):
        opening_arguments(llm, user_input_debate_topic, prompts["debater_one_start_prompt"], prompts["summarize_prompt"], debate_dictionary, "favor", user_input_style, pro_placeholder, con_placeholder)
    
    with st.spinner("Preparing my opening remarks ..."):
        opening_arguments(llm, user_input_debate_topic, prompts["debater_two_start_prompt"], prompts["summarize_prompt"], debate_dictionary, "opposition", user_input_style, pro_placeholder, con_placeholder)
    
    for i in range(user_input_number_of_rounds):

        with st.spinner(generate_loading_statements()):
            debate(llm, user_input_debate_topic, prompts["debater_one_follow_up_prompt"], prompts["summarize_prompt"], debate_dictionary, debate_dictionary["opposition"], debate_dictionary["favor"], "favor", user_input_style, pro_placeholder, con_placeholder)        
        
        with st.spinner(generate_loading_statements()):
            debate(llm, user_input_debate_topic, prompts["debater_one_follow_up_prompt"], prompts["summarize_prompt"], debate_dictionary, debate_dictionary["favor"], debate_dictionary["opposition"], "opposition", user_input_style, pro_placeholder, con_placeholder)

    with st.spinner("Wrapping up the debate ..."):
        st.header("Host summary:")
        final_statement = llm(prompts["host_end_prompt"].format(
